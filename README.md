...
- Run ```me doctor``` to validate every manifest, managed-file marker, and destination claim. Run ```me plan yojam``` (or ```me plan app```) to validate and preview actions without deploying them.
